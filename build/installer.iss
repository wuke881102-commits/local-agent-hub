...
[InstallDelete]
; 5.5 起主程序改名 LocalAgentHub.exe：升级时清掉 5.4 及更早版本留下的旧 exe。
Type: files; Name: "{app}\backend\feishu-agent.exe"
...
begin
  ...
  begin
    // 覆盖文件前先结束运行中的实例（含 5.4 及更早的旧 exe 名），
    // 否则同名 exe 被占用会导致升级失败；用户无需记得先手动退出。
