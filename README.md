...
| **A9** | **本地内容** | 把本机截图 / 图片交给视觉模型识别，直出 HTML 或结构化要点。 | 本机图片 → HTML / 要点 |
| **A10** | **AIHot 模型榜** | 抓公开站点的模型共识榜，按性价比重排并给出选型建议。 | 公开榜单 → 排名 + 选型建议 |
| **A11** | **AIHot 新闻简报** | 抓公开站点的 AI 新闻与日报，合并成一页可下载的简报。 | 公开新闻 → 简报页 |

> 另有 **📸 自动化提炼**（见下）—— 它不是 Agent，是一条后台留痕 + 定时提炼的流水线。
>
> A10 / A11 抓的是公开站点：只走站点的公开接口与页面，UA 诚实标 `LocalAgentHub/<版本>`、不伪装浏览器；站点声明商用须先取得书面授权，所以产出页的页脚会自动带上这条提示。
