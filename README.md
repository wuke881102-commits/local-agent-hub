...
工作台区分两类对象：**飞书在线对象**（有内容接口，直接读）和 **云盘上传文件**（二进制，先下载到本机再用对应解析器抽取）。

| 类型 | 来源 | 场景 | 工作方式 |
|---|:--:|---|---|
| **飞书文档** `docx·doc·wiki` | 🟢 在线 | 内容生产 / 知识治理 | 抽取正文（含内嵌图识别）→ 重组 HTML，或生成摘要 / 标签 |
| **多维表格** `bitable` | 🟢 在线 | 表格分析 | 读全部子表 → 列画像 → AI 出图，数字由本地精确聚合 |
| **电子表格** `sheet` | 🟢 在线 | 表格分析 | 逐工作表出图，支持「问数据」自然语言查询 |
| **演示文稿** `slides` | 🟢 在线 | 内容生产 | 抽取页面文字 → 重组 HTML |
| **会议 / 妙记** `meeting` | 🟢 在线 | 会议沉淀 | 整理成摘要、决策、待办、风险 |
| **PDF** `.pdf` | 🔵 上传 | PDF 识别 | 逐页抽取正文与表格，扫描件走 OCR → 字段 / 金额测算 |
| **Excel** `.xlsx .xls` | 🔵 上传 | 表格分析 | 本地解析每个工作表 → 出图与「问数据」 |
| **Word** `.docx` | 🔵 上传 | 内容生产 | 本地解析段落与表格 → 重组 HTML |
| **网页** `.html .htm` | 🔵 上传 | 内容生产 | 提取正文 → 重组 HTML |

> 解析全部在本地完成（Excel→openpyxl，Word→python-docx，网页→BeautifulSoup，PDF→PyMuPDF），不外传。
