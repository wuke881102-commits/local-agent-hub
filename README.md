...
- 🎙️ **语音速记** — 按下才录（麦克风 / 系统声音 / 两路同时），说完出一条逐字稿 + 提炼；也可以传一段已有的会议录音或视频进来转写。**音频没有任何一条代码路径写到磁盘**，存下来的只有文字。
- 📸 **自动留痕 + 定时提炼** — 工作时按 Enter 自动截当前窗口（存本机私有目录），每隔一段时间用视觉模型提炼「这段时间在做什么」，单次最长 10 小时自动停止；和「个人摘记」合并成一页**个人提炼**。
...
| **A3** | **知识治理** | 找出重复、过期、缺负责人的文档，并给出处理建议。 | 资产索引 → 治理清单与建议 |
| **A4** | **多维表格分析** | 表格数据归纳并出图（柱状 / 饼 / 折线 / 甘特 / 架构图，可下载）；一次分析全部子表，支持「问数据」自然语言查询。 | 多维表 / 电子表 / Excel → 图表 + 洞察 |
| **A5** | **PDF 识别** | 识别云盘 PDF 全文、字段与表格，扫描页走 OCR，支持逐页识别与合同金额测算。 | 云盘 PDF → 结构化文本 / 字段 / 金额 |
| **A6** | **会议纪要** | 把妙记或会议记录整理成摘要、决策、待办和风险点，待办可写回飞书任务。 | 会议 / 妙记 → 结构化纪要 |
| **A7** | **协作分发** | 生成消息、邮件、任务和日程提醒的草稿，确认后再发送。 | 成果 → 消息 / 邮件 / 任务草稿 |
| **A8** | **本地内容** | 把本机截图 / 图片交给视觉模型识别，直出 HTML 或结构化要点。 | 本机图片 → HTML / 要点 |
| **A9** | **AIHot 模型榜** | 抓公开站点的模型共识榜，按性价比重排并给出选型建议。 | 公开榜单 → 排名 + 选型建议 |
| **A10** | **AIHot 新闻简报** | 抓公开站点的 AI 新闻与日报，合并成一页可下载的简报。 | 公开新闻 → 简报页 |

> **摘要与标签回填**不再单列成 Agent —— 它现在随「刷新索引」自动在后台跑一次，不用记得手点。跳过时会说明原因（模型是 Mock / 没有待补的 / 已经在跑）。
>
> 另有 **🎙️ 语音速记** 与 **📸 个人提炼**（见下）—— 都不是 Agent：前者是录音/转写，后者是一条后台留痕 + 定时提炼 + 按频率汇总发出的流水线。
>
> A9 / A10 抓的是公开站点：只走站点的公开接口与页面，UA 诚实标 `LocalAgentHub/<版本>`、不伪装浏览器；站点声明商用须先取得书面授权，所以产出页的页脚会自动带上这条提示。
...
| **个人提炼** | 按 Enter 自动留痕，定时提炼成「工作说明 / 重点 / 操作 / 会议」，再按频率汇总成一条发到你自己的飞书 | 个人摘记 · 自动化提炼 |
...
| 🔵 **音频 / 视频** `.m4a .mp3 .wav .aac .flac .ogg .opus .mp4 .mov` | 语音速记 | 字节直接进内存解码（**不落盘**）→ 分段转写 → 逐字稿 + 提炼 |
